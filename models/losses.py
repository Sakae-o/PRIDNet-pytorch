...
class total_variation_loss(nn.Module):
    ...
    def forward(self, input):
        dh = input[:, :, :-1, ...] - input[:, :, 1:, ...]
        dw = input[:, :, :, :-1] - input[:, :, :, 1:]
        return torch.norm(dh) / dh.nelement() + torch.norm(dw) / dw.nelement()
...
if __name__ == '__main__':
    import cv2
    from network import PRIDNet

    img = cv2.imread('D://CV//Denoise//dataset//train//noise//00121.jpg').transpose(2, 0, 1) / 255
    img2 = cv2.imread('D://CV//Denoise//dataset//train//origin//00121.jpg', 0)
    img2 = img2[None, None, :]
    img = img[None, :]
    ...
    loss = MS_SSIM(channel=1)
    loss1 = nn.L1Loss()

    img3 = cv2.imread('D://CV//Denoise//dataset//train//origin//00021.jpg', 0)
    img3 = img3[None, None, :]
    img3 = torch.tensor((img3), dtype=torch.float)
    print(loss1(img2, P(img)))
    print(loss(img2, P(img)))
